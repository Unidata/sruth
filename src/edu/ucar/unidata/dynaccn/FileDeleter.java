 ...
 */
@NotThreadSafe
final class FileDeleter implements Callable<Void> {
    /**
     ...
     */
    private final FileDeletionQueue queue;
    /**
     ...
     */
    FileDeleter(final Path rootDir, final FileDeletionQueue queue) {
    ...
    private void delete(final Path path) throws IOException {
        ...
        for (Path dir = path.getParent(); dir != null
                && !dir.isSameFile(rootDir); dir = dir.getParent()) {
            if (isEmpty(dir)) {
                try {
                    dir.delete();
                }
